...
| 10  | **No lead persistence fallback** — with zero integration keys a lead is only a console line                                                                                                                                  | **FIXED** — `src/lib/lead-store.ts`: every valid form/phone lead persists to a **private** Vercel Blob store (KV is deprecated → Marketplace/Upstash). Env-gated on `BLOB_READ_WRITE_TOKEN`, never blocks the lead; owner setup: `docs/setup-leads.md` §3                         |
| 11  | Phone leads skip Meta CAPI; CallRail webhook has no rate limit                                                                                                                                                               | **FIXED** — webhook fires `sendMetaLeadEvent` (`action_source: phone_call`, CallRail call id as dedup `event_id`) + same rate limit as `/api/lead`                                                                                                                                |
...
| 14  | Content-pipeline GitHub Actions crons invoke stubbed scripts                                                                                                                                                                 | **FIXED** — `compliance-scan.ts` implemented (banned-phrase scan of `dist/`, was failing + auto-filing a false `[COMPLIANCE]` issue every Monday); brief/draft/e2e workflows get fail-fast guard steps; `reviews-sync` documented as blocked on GBP (see `src/scripts/README.md`) |
...
- **P2 (30–90 days)** — ~~lead persistence fallback~~, ~~implement-or-disable
  the stubbed content-pipeline crons~~, ~~Meta CAPI for phone leads + webhook
  rate limit~~ (all three shipped post-review — see items 10, 11, 14);
  revenue-attribution sheet (manual first) → offline conversion uploads;
  storm kit; JobNimbus with attribution-carry SOP.
